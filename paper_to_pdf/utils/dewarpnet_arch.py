...
出典: https://github.com/cvlab-stonybrook/DewarpNet (MIT License)

2 モデル構成:
  unetnc   : UNet (形状予測 — 入力画像 → 3D 変形座標 WC)
  dnetccnl : DenseNet (後退マッピング予測 — WC → 逆変換マップ BM)
"""
...
# ──────────────────────────────────────────────
# state_dict の DataParallel プレフィックス除去
# ──────────────────────────────────────────────

def convert_state_dict(state_dict: dict) -> OrderedDict:
    """DataParallel で保存された state_dict の 'module.' プレフィックスを除去する。"""
    new = OrderedDict()
...
class UnetGenerator(nn.Module):
    """pix2pix スタイルの UNet (unetnc — WC 予測用)。"""
...
# ──────────────────────────────────────────────
# DenseNet + CoordConv (dnetccnl)
# ──────────────────────────────────────────────

def _add_coord_channels(t: torch.Tensor) -> torch.Tensor:
    """CoordConv: H/W 座標チャンネルを追加する ([-1, 1] 正規化)。"""
    n, c, h, w = t.size()
...
class _DenseBlockEncoder(nn.Module):
    def __init__(self, n_ch, n_convs):
        ...
        self.layers = nn.ModuleList([
            nn.Sequential(nn.BatchNorm2d(n_ch), nn.ReLU(False),
...
class _DenseBlockDecoder(nn.Module):
    def __init__(self, n_ch, n_convs):
        ...
        self.layers = nn.ModuleList([
            nn.Sequential(nn.BatchNorm2d(n_ch), nn.ReLU(False),
                          nn.ConvTranspose2d(n_ch, n_ch, 3, padding=1, bias=False))
            for _ in range(n_convs)
        ])
...
class _DenseTransEnc(nn.Module):
    def __init__(self, in_ch, out_ch, mp):
        ...
        self.main = nn.Sequential(
            nn.BatchNorm2d(in_ch), nn.LeakyReLU(0.2, False),
            nn.Conv2d(in_ch, out_ch, 1, bias=False), nn.MaxPool2d(mp),
        )
...
class _DenseTransDec(nn.Module):
    def __init__(self, in_ch, out_ch):
        super().__init__()
        self.main = nn.Sequential(
            nn.BatchNorm2d(in_ch), nn.ReLU(False),
...
class _WaspDenseEncoder128(nn.Module):
    def __init__(self, nc, ndf, ndim):
        ...
        self.main = nn.Sequential(
            ...
            _DenseBlockEncoder(ndf, 6),   _DenseTransEnc(ndf,    ndf*2, 2),
            _DenseBlockEncoder(ndf*2, 12), _DenseTransEnc(ndf*2,  ndf*4, 2),
            _DenseBlockEncoder(ndf*4, 16), _DenseTransEnc(ndf*4,  ndf*8, 2),
            _DenseBlockEncoder(ndf*8, 16), _DenseTransEnc(ndf*8,  ndf*8, 2),
            _DenseBlockEncoder(ndf*8, 16), _DenseTransEnc(ndf*8,  ndim,  4),
            nn.Tanh(),
        )
    def forward(self, x):
...
class _WaspDenseDecoder128(nn.Module):
    def __init__(self, nz, nc, ngf):
        super().__init__()
        self.main = nn.Sequential(
            nn.BatchNorm2d(nz), nn.ReLU(False),
            nn.ConvTranspose2d(nz, ngf*8, 4, 1, 0, bias=False),
            _DenseBlockDecoder(ngf*8, 16), _DenseTransDec(ngf*8, ngf*8),
            _DenseBlockDecoder(ngf*8, 16), _DenseTransDec(ngf*8, ngf*4),
            _DenseBlockDecoder(ngf*4, 12), _DenseTransDec(ngf*4, ngf*2),
            _DenseBlockDecoder(ngf*2, 6),  _DenseTransDec(ngf*2, ngf),
            _DenseBlockDecoder(ngf,   6),  _DenseTransDec(ngf,   ngf),
            nn.BatchNorm2d(ngf), nn.ReLU(False),
...
class DnetCCNL(nn.Module):
    """DenseNet + CoordConv + 全結合 (dnetccnl — BM 予測用)。"""
