...
class RoutesTest(unittest.TestCase):
    ...
    def test_index_with_genre_filter(self):
        with patch("app.routes._shared.fetch_program_list_async", new_callable=AsyncMock, return_value=[]) as m:
    ...
    def test_index_with_unclassified_filter(self):
        with patch(
            "app.routes._shared.fetch_program_list_async",
    ...
    def test_settings_post_saves_storage_limit(self):
        ...
        with patch("app.routes._shared.save_storage_limit", return_value=True) as mock_save:
            resp = self.client.post(
                "/api/settings",
    ...
    def test_settings_post_save_failure(self):
        ...
        with patch("app.routes._shared.save_storage_limit", return_value=False):
            resp = self.client.post(
                "/api/settings",
    ...
    def test_api_v1_settings_get(self):
        with patch("app.routes.api_v1.load_storage_limit", return_value=3 * 1024 * 1024 * 1024):
