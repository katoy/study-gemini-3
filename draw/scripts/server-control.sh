...
# Draw アプリのバックエンド（Express/WebSocket）サーバー管理スクリプト
...
case "${1:-status}" in
  ...
  *)
    ...
    echo ""
    echo "コマンド:"
    echo "  start   - バックエンドサーバーをバックグラウンドで起動"
    echo "  stop    - バックエンドサーバーを停止"
    echo "  restart - バックエンドサーバーを再起動"
    echo "  status  - バックエンドサーバーの状態を確認"
    exit 1
    ;;
esac
