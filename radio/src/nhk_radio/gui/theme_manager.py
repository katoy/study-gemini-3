...
class ThemeManager:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.settings = _load_ui_settings()
        self.current_theme = self.settings.get("theme", DEFAULT_UI_THEME)
        self.current_font_size = self.settings.get("font_size_pt", DEFAULT_UI_FONT_SIZE_PT)
    ...
    def save_settings(self, theme: str, font_size: int, search_history: list[str]):
        ...
        _save_ui_settings(theme, font_size, search_history)
        self.settings = {
            "theme": theme,
    ...
    def apply_theme(self, theme_name: str | None = None, font_size: int | None = None):
        ...
        # Specific widgets (Condensed from styling.py)
    ...
    def _get_palette(self, theme_name: str) -> dict[str, str]:
        if theme_name == "dark":
            return {
                ...
                "row_odd": "#1A1A1A", "dl_even": "#1B281B", "dl_odd": "#1E2F1E", 
        ...
        return {
            ...
            "row_odd": "#F1F3F5", "dl_even": "#E8F5E9", "dl_odd": "#DBEDDE", 
    ...
    def _configure_treeviews(self, p, f):
        self.style.configure("Treeview", font=f["mono"], rowheight=f["rowheight"], background=p["surface"], foreground=p["text"], fieldbackground=p["surface"], bordercolor=p["border_strong"])
        self.style.configure("Treeview.Heading", font=f["ui_bold"], background=p["surface_alt"], foreground=p["text"], relief="solid", padding=(10, 9), bordercolor=p["border_strong"], borderwidth=1)
        self.style.map("Treeview", background=[("selected", p["selected_bg"])], foreground=[("selected", p["selected_fg"])])
    ...
    def _configure_inputs(self, p, f):
        ...
        self.style.configure("Search.TCombobox", fieldbackground=p["input_bg"], background=p["input_bg"], foreground=p["text"])
